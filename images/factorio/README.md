...
image. The only change is an entrypoint wrapper that keeps the upstream Bash
trace stream out of container logs. This is required because the upstream
entrypoint expands its generated RCON password while tracing commands.
...
The Docker build verifies that hash before adding the wrapper.
